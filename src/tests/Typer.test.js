...
test('User can backspace', () => {
  render(<Typer text="Hello, World" />);
  userEvent.type(screen.getByTestId('inputBox'), "Hello, World");
  expect(within(screen.getByTestId('text-box')).getByText(/d/)).toHaveClass("letter-correct");
  userEvent.type(screen.getByTestId('inputBox'), "Hello, Worl");
  expect(within(screen.getByTestId('text-box')).getByText(/d/)).toHaveClass("letter-active");
  userEvent.type(screen.getByTestId('inputBox'), "Hello, Wor");
  expect(within(screen.getByTestId('text-box')).getByText(/d/)).toHaveClass("letter-not-entered");
});
